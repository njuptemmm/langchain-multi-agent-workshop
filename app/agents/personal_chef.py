...
# 4.初始化checkpointer
# 连接sqlite（路径基于项目根目录，避免受启动工作目录影响）
_db_dir = Path(__file__).resolve().parent.parent.parent / "db"
_db_dir.mkdir(parents=True, exist_ok=True)
connection = sqlite3.connect(_db_dir / "personal_chief.db", check_same_thread=False)
# 初始化checkpointer
...
agent = create_agent(
    model=model,  # 模型
    tools=[web_search],  # 工具
    checkpointer=checkpointer,  # 记忆初始化
    system_prompt=system_prompt  # 系统提示词
)

# 流式对话
async def search_recipes(prompt: str, image: str, thread_id: str):#前端发回的请求就是这三个参数
    """调用agent搜索食谱"""
